...
**当前版本：[v0.1.71](https://github.com/a3165458/lighting/releases/tag/v0.1.71)**（仅平板恢复后保留虚拟扩展屏）
...
| 文件 | 说明 |
|------|------|
| `Lighting-0.1.71-setup.exe` | 安装包（推荐） |
| `Lighting-0.1.71-portable.exe` | 便携版，不用安装 |
...
2. 安装 `Lighting-0.1.71-setup.exe`，或打开便携包
...
验证边界：已通过本地 146 项主机测试。尚未在受影响电脑上验证休眠唤醒后的虚拟屏是否重新出现。升级后请确认显示器列表同时有主屏和虚拟屏，再开始共享。
