...
平板**不会**出现在 Win+P 投影列表里——Lighting 是抓取 Windows 已有显示器画面再推到平板，而不是把平板注册成系统显示器。

| 模式 | 作用 | 相当于 Win+P |
|------|------|--------------|
| 镜像主屏 | 平板与电脑主屏同画面 | 复制 |
| 扩展屏（推荐） | 平板显示独立桌面 | 扩展 |
| 仅第二屏 | 桌面只出现在第二屏 | 仅第二屏幕 |

真正「扩展」需要一块**虚拟显示器**（或第二块物理屏），再由 Lighting 抓取该屏推到平板：

```powershell
winget install --id=VirtualDrivers.Virtual-Display-Driver -e
```

安装后在 **设置 → 系统 → 显示器** 中设为「扩展这些显示器」，分辨率可设 2560×1440。在 Lighting 选「扩展屏」模式后点「开始共享」（会自动选虚拟屏）。

没有虚拟屏时请先用「镜像主屏」验证编码和 USB 通道。
