...
扩展屏功能需要安装开源 **MttVDD** 虚拟显示驱动。  
杀软看到「静默装驱动 + 管理员权限」时更容易报警——这与华硕 GlideX 等商业软件同类，商业软件靠签名和商店信誉减轻提示；开源未签名包则更容易中招。

**结论：**  
- **用户侧**：点「仍要运行」/ 加信任即可用  
- **产品侧**：配置 `WINDOWS_CSC_*` 证书签名，才是消掉 SmartScreen「发布者未知」和大幅减少 360 误报的正道  
